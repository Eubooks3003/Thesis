...
def modified_render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    print("opacities NaN:", torch.isnan(opacity).any(), "Inf:", torch.isinf(opacity).any())
    ...
    print("Predicted image contains NaN Rendered : ", torch.isnan(rendered_image).any())
    print("Predicted image contains Inf Modified : ", torch.isinf(rendered_image).any())
    print("Predicted image shape: ", rendered_image.shape)
    print("")
